...
/* ── Per-diagram allowed category groups ─────────────────── */
const DIAGRAM_CATEGORIES = {
  use_case: ['actor', 'use_case', 'system', 'note', 'constraint'],
  activity: ['start', 'action', 'decision', 'end', 'note', 'constraint'],
  dfd:      ['external', 'process', 'data_store', 'note'],
  sequence: ['lifeline', 'note'],
  class:    ['class', 'interface', 'note'],
};
// All categories (fallback when no diagram type is set)
const ALL_CATEGORIES = ['actor', 'use_case', 'process', 'data_store', 'decision', 'note', 'constraint', 'start', 'end', 'external', 'system'];
...
export default function NodePanel({ 
  ...
  useEffect(() => {
    ...
    const grouped = useMemo(() => {
        const map = {};
        // Use diagram-type-specific order, falling back to all categories
        const order = (diagramType && DIAGRAM_CATEGORIES[diagramType]) || ALL_CATEGORIES;
        order.forEach(cat => map[cat] = []);

        nodes.forEach(n => {
            const cat = n.category || 'use_case';
            if (!map[cat]) map[cat] = [];  // allow unknown cats from AI
            map[cat].push(n);
        });
        return map;
    }, [nodes, diagramType]);
                                    ...
                                    {catNodes.map(node => {
                                        ...
                                        return (
                                            <div
                                                ...
                                                onClick={() => {
                                                    if (!placed && onSpawnNode) {
                                                        const resolvedJointType = node.jointType || meta.jointType;
                                                        onSpawnNode({ ...node, jointType: resolvedJointType });
                                                    }
                                                }}
                                                style={{
